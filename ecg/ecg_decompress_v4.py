...
from ecg.AICode import AIModelProcessor
import ecg.demo_rf_model.demo_rf_model.rf_predict as rf_predict
from ecg.unified_ecg_system.ecg_model.signal_manager import SignalManager
from ecg.unified_ecg_system.ecg_model.poting_model import PotingModel
from ecg.api import ArrayData, getTokenForRequest, resetOldVisualization, sendData
from ecg.map import save_fft_mfcc_spectrogram
...
def process_and_send_er3_data(
    ...
    # Initialize model(s) based on selection
    ai_processor = None
    poting_model = None
    signal_manager = None
    ...
    if model_type == "rf":
        ai_processor = AIModelProcessor(
            model_dir="./ecg/demo_rf_model/demo_rf_model/2026-01-29_10-06-37/models/fold_2",
            sampling_rate=sampling_rate,
        )
    elif model_type == "poting":
        signal_manager = SignalManager(sampling_rate=sampling_rate)
        poting_model = PotingModel(device="cpu")
    ...
    for start in range(0, total_samples, chunk_size):
        ...
        ai_results = None
        if model_type == "rf" and ai_processor is not None:
            ai_results = ai_processor.process_new_chunk(raw_signals_chunk)
        elif model_type == "poting" and poting_model is not None and signal_manager is not None:
            # Use unified system's feature pipeline + PoTing model
            signal_manager.append_signals(raw_signals_chunk['I'], [])
            signal_manager.process_signals()
            featureset = signal_manager.get_stage2_features()
            poting_probs = poting_model.predict(featureset)  # shape (1, num_classes)
            threshold = 0.5
            poting_binary = np.where(np.array(poting_probs) >= threshold, 1, 0)
            ai_results = (poting_binary, poting_probs)
...
def main():
    ...
    parser.add_argument(
        ...
        choices=['rf', 'poting'],
        default='rf',
        help='Select AI model: "rf" (random forest) or "poting"',
    )
    ...
    process_and_send_er3_data(
        raw_data=raw,
        sampling_rate=250,
        description=str(input_path.name),
